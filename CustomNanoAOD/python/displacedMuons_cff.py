...
def add_displaced_muons(process):
    ...
    process.displacedMuonTable = simplePATMuonFlatTableProducer.clone(
        ...
        variables = cms.PSet(CandVars,
            ...
            pfIso04_sumChargedHadronPt = Var("pfIsolationR04().sumChargedHadronPt", float, doc="PF isolation R=0.4, charged hadron pT sum [GeV]", precision=10),
            pfIso04_sumPUPt         = Var("pfIsolationR04().sumPUPt", float, doc="PF isolation R=0.4, PU charged hadron pT sum [GeV]", precision=10),
            pfIso04_sumNeutral      = Var("pfIsolationR04().sumNeutralHadronEt + pfIsolationR04().sumPhotonEt", float, doc="PF isolation R=0.4, neutral hadron + photon ET sum [GeV]", precision=10),
            timeNdof      = Var("time().nDof", int, doc="muon time ndof"),
            timeAtIpInOut = Var("time().timeAtIpInOut", float, doc="muon time at IP (in-out) [ns]", precision=10)
        ),
    )
